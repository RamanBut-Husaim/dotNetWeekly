# .NET Weekly #23 (13.02 - 19.02)

## Cloud & Web

1. [JavaScript Start-up Performance](https://medium.com/dev-channel/javascript-start-up-performance-69200f43b201#.xb1geiosv)
1. [An in-depth look at the various ways of specifying the IP or host ASP.NET Core listens on](http://josephwoodward.co.uk/2017/02/many-different-ways-specifying-host-port-asp-net-core)
1. [HTTP/2 Server Push and ASP.NET MVC - Cache Digest](http://tpeczek.blogspot.de/2017/01/http2-server-push-and-aspnet-mvc-cache.html)

## Tooling & Libraries

1. [Introduction to Distributed Publish-Subscribe in Akka.NET](https://petabridge.com/blog/distributed-pub-sub-intro-akkadotnet/)
1. [Refactoring Towards Resilience: Evaluating Coupling](https://jimmybogard.com/refactoring-towards-resilience-evaluating-coupling/)
1. [Choosing centralized logging and monitoring system](https://indexoutofrange.com/Choosing-centralized-logging-and-monitoring-system/)

## Architecture & Databases

1. [An alternative take on application architecture](http://bart-sokol.info/blog/2017/02-05-an-alternative-take-on-application-architecture/index.html)
1. [SQL Server Encryption: Always Encrypted](https://www.simple-talk.com/sql/database-administration/sql-server-encryption-always-encrypted/)
1. [FunctionAsObject](https://martinfowler.com/bliki/FunctionAsObject.html)
1. [The Good of Event Sourcing: Projections](https://dzone.com/articles/the-good-of-event-sourcing-projections)
1. [Hexagonal Architecture Is Powerful](https://dzone.com/articles/hexagonal-architecture-is-powerful)

## Programming Languages

1. [C# 7.0 – Pattern Matching](https://csharp.christiannagel.com/2017/02/15/patternmatching/)

## Runtime

1. [A bug story about named mutex on Mono](http://aakinshin.net/en/blog/dotnet/namedmutex-on-mono/)
1. [65535 interfaces ought to be enough for anybody](http://aakinshin.net/en/blog/dotnet/mono-and-65535interfaces/)
1. [.NET Core Roadmap](https://github.com/dotnet/core/blob/master/roadmap.md)

## Other

1. [StackOverflow Is Your Mandatory Tool](http://www.yegor256.com/2017/02/14/stackoverflow-is-your-tool.html)
1. [Introducing Haskelly – Haskell extension for Visual Studio Code](https://blogs.msdn.microsoft.com/uk_faculty_connection/2017/02/13/introducing-haskelly-extension-for-visual-studio-code/)
1. [Google releases TensorFlow 1.0 with new machine learning tools](http://venturebeat.com/2017/02/15/google-releases-tensorflow-1-0-with-new-machine-learning-tools/)
1. [Go 1.8 Release Notes](https://beta.golang.org/doc/go1.8)